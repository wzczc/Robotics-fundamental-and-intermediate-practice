...
void OLED_ShowChar(uint8_t x,uint8_t y,uint8_t chr,uint8_t sizey)
{
  ...
  for(i=0;i<size1;i++)
  {
    ...
    if(sizey==8) OLED_WR_Byte(asc2_0806[c][i],OLED_DATA);//6X8字号
            ...
            else if(sizey==12) OLED_WR_Byte(asc2_1206[c][i],OLED_DATA);//6x12字号
    else return;
  }
}
//m^n函数
u32 oled_pow(uint8_t m,u8 n)
{
  u32 result=1;
  while(n--)result*=m;
  return result;
...
void OLED_ShowNum(u8 x,u8 y,u32 num,u8 len,u8 sizey)
{
  ...
  for(t=0;t<len;t++)
  {
    ...
    if(enshow==0&&t<(len-1))
    {
      if(temp==0)
      {
        OLED_ShowChar(x+(sizey/2+m)*t,y,'0',sizey);
        continue;
      }else enshow=1;
    }
...
void OLED_ShowChinese(u8 x,u8 y,u8 no,u8 sizey)
{
 ...
 //       else if(sizey==12) OLED_WR_Byte(Hzk1[no][i],OLED_DATA);//用户添加字号
    else return;
  }
}
